...
ENSEMBLE_JOB_SCHEMA = "merlino.semiexp.ensemble.v1"
...
def fit_ensemble_class_corrections(
    ...
    *,
    ...
    with TemporaryDirectory(prefix="merlino_ensemble_gicforge_") as tmp:
        root = Path(tmp)
...
def read_ensemble_job(path: Path) -> EnsembleJobInput:
    ...
    if data.get("schema") != ENSEMBLE_JOB_SCHEMA:
        raise ValueError(f"Ensemble job must declare schema = {ENSEMBLE_JOB_SCHEMA!r}")
