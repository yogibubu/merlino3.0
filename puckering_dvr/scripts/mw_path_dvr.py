...
DEFAULT_GAUSSIAN_INPUTS = ROOT / "gaussian_inputs"
...
def parse_five_ring_pucker_label(label: str) -> tuple[str, int, float]:
    ...
    Accepted envelope labels are E1/E_1 and ^1E/E^1, generalized to atoms 1..5.
    Accepted twist labels are T1/T_1 and ^1T/T^1, generalized to atoms 1..5.
    E1 is phi=0 by convention. T1 is halfway between E1 and E2.
    """
    raw = label.strip()
    compact = raw.replace("_", "").replace("{", "").replace("}", "").replace(" ", "").upper()
    if not compact:
        raise ValueError("Empty puckering label")

    opposite = compact.startswith("^") or "^" in compact or compact[0].isdigit()
    compact = compact.replace("^", "")

    match = re.fullmatch(r"([ET])([1-5])", compact)
    if match:
        kind = match.group(1)
        atom = int(match.group(2))
    else:
        match = re.fullmatch(r"([1-5])([ET])", compact)
        if not match:
            raise ValueError(
                f"Unsupported puckering label {label!r}; use E1, ^1E, T1, or ^1T "
                "with atom numbers 1..5"
            )
    ...
    if opposite:
        base += 180.0
    return kind, atom, base % 360.0


def five_ring_plane(coords: np.ndarray, ring_indices: list[int]) -> tuple[np.ndarray, np.ndarray]:
    ring = coords[ring_indices]
    center = np.mean(ring, axis=0)
    j = np.arange(5, dtype=float)
    angle1 = 2.0 * math.pi * j / 5.0
    ...
    norm = float(np.linalg.norm(normal))
    if norm < 1.0e-12:
        _, _, vh = np.linalg.svd(ring - center, full_matrices=False)
        normal = vh[-1]
        norm = float(np.linalg.norm(normal))
    if norm < 1.0e-12:
        raise ValueError("Cannot define a mean plane for the selected five-membered ring")
    return center, normal / norm


def target_five_ring_z(amplitude: float, phi_deg: float) -> np.ndarray:
    j = np.arange(5, dtype=float)
    angle = 4.0 * math.pi * j / 5.0
    phi = math.radians(phi_deg)
    return math.sqrt(2.0 / 5.0) * amplitude * np.cos(angle + phi)


def angular_difference_deg(angle: float, reference: float) -> float:
    return (angle - reference + 180.0) % 360.0 - 180.0


def nearest_ring_locals(coords: np.ndarray, ring_indices: list[int]) -> np.ndarray:
    ring_index_array = np.array(ring_indices)
    atom_to_ring = np.argmin(
        np.linalg.norm(coords[:, None, :] - coords[ring_index_array][None, :, :], axis=2),
        axis=1,
    )
    for local, atom_index in enumerate(ring_indices):
        atom_to_ring[atom_index] = local
    return atom_to_ring


def retarget_ring_z_displacements(
    coords: np.ndarray,
    atom_to_ring: np.ndarray,
    normal: np.ndarray,
    current_z: np.ndarray,
    target_z: np.ndarray,
) -> np.ndarray:
    ring_delta = target_z - current_z
    new_coords = coords.copy()
    new_coords += ring_delta[atom_to_ring][:, None] * normal
    return new_coords


def cartesian_guess_phase_for_torsion_phase(
    coords: np.ndarray,
    ...
    def torsion_phase(cp_phase: float) -> float:
        trial_coords = retarget_ring_z_displacements(
            coords,
            atom_to_ring,
            normal,
            current_z,
            target_five_ring_z(amplitude, cp_phase),
        )
        torsions = five_ring_endocyclic_torsions(trial_coords, ring_indices)
        return five_ring_components(torsions)[3]

    grid = np.linspace(0.0, 360.0, 721)
    best_phase = min(
        grid,
        key=lambda phase: abs(angular_difference_deg(torsion_phase(float(phase)), target_phi_deg)),
    )

    def objective(cp_phase: float) -> float:
        diff = angular_difference_deg(torsion_phase(cp_phase % 360.0), target_phi_deg)
        return diff * diff

    result = minimize_scalar(
        objective,
        bounds=(float(best_phase) - 2.0, float(best_phase) + 2.0),
        method="bounded",
        options={"xatol": 1.0e-8},
    )
    return float(result.x) % 360.0


def pucker_structure_from_label(
    structure: Structure,
    ring_indices: list[int],
    label: str,
    amplitude: float | None,
) -> Structure:
    kind, atom, phi_deg = parse_five_ring_pucker_label(label)
    coords = structure.coords_angstrom.copy()
    center, normal = five_ring_plane(coords, ring_indices)
    ring = coords[ring_indices]
    current_z = (ring - center) @ normal
    cp_q2, _, _, _ = cremer_pople_five_membered(coords, ring_indices)
    target_amplitude = amplitude if amplitude is not None else cp_q2
    if target_amplitude < 1.0e-6:
        target_amplitude = 0.35
    atom_to_ring = nearest_ring_locals(coords, ring_indices)
    cartesian_phase = cartesian_guess_phase_for_torsion_phase(
        coords,
        ring_indices,
        atom_to_ring,
        normal,
    ...
    target_z = target_five_ring_z(target_amplitude, cartesian_phase)
    new_coords = retarget_ring_z_displacements(coords, atom_to_ring, normal, current_z, target_z)

    new_structure = Structure(
        atoms=structure.atoms.copy(),
        symbols=list(structure.symbols),
        coords_angstrom=new_coords,
        props=dict(structure.props),
        comment=(
            f"{structure.comment} start_pucker_label={label} "
            f"start_pucker_kind={kind} start_pucker_atom={atom} "
            f"start_pucker_phi_deg={phi_deg:.6f} "
            f"start_pucker_cartesian_phase_deg={cartesian_phase:.6f} "
            f"start_pucker_q_angstrom={target_amplitude:.6f}"
        ).strip(),
    )
    new_structure.props["start_pucker_phi_deg"] = phi_deg
    new_structure.props["start_pucker_cartesian_phase_deg"] = cartesian_phase
    new_structure.props["start_pucker_q_angstrom"] = target_amplitude
    new_structure.props["start_pucker_label"] = label
    new_structure.props["start_pucker_kind"] = kind
    new_structure.props["start_pucker_atom"] = atom
    return new_structure


def parse_pucker_label(label: str, ring_size: int) -> tuple[str, int, float]:
    if ring_size == 5:
        return parse_five_ring_pucker_label(label)
    if ring_size == 4:
        return parse_four_ring_pucker_label(label)
    raise ValueError("Only four- and five-membered rings are supported")


def label_structure_for_gic_start(structure: Structure, label: str, ring_size: int) -> Structure:
    kind, atom, phi_deg = parse_pucker_label(label, ring_size)
    new_structure = Structure(
        atoms=structure.atoms.copy(),
        symbols=list(structure.symbols),
        coords_angstrom=structure.coords_angstrom.copy(),
        props=dict(structure.props),
        comment=(
            f"{structure.comment} start_pucker_label={label} "
            f"start_pucker_kind={kind} start_pucker_atom={atom} "
            f"start_pucker_phi_deg={phi_deg:.6f} start_pucker_mode=gic"
        ).strip(),
    )
    new_structure.props["start_pucker_phi_deg"] = phi_deg
    new_structure.props["start_pucker_label"] = label
    new_structure.props["start_pucker_kind"] = kind
    new_structure.props["start_pucker_atom"] = atom
    new_structure.props["start_pucker_mode"] = "gic"
    return new_structure


def parse_four_ring_pucker_label(label: str) -> tuple[str, int, float]:
    raw = label.strip()
    compact = raw.replace("_", "").replace("{", "").replace("}", "").replace(" ", "").upper()
    if not compact:
        raise ValueError("Empty puckering label")
    opposite = compact.startswith("^") or "^" in compact or compact[0].isdigit()
    compact = compact.replace("^", "")
    match = re.fullmatch(r"E([1-4])", compact)
    if match:
        atom = int(match.group(1))
    else:
        match = re.fullmatch(r"([1-4])E", compact)
        if not match:
            raise ValueError(
                f"Unsupported four-membered-ring label {label!r}; use E1 or ^1E "
                "with atom numbers 1..4"
            )
        atom = int(match.group(1))
        opposite = True
    if atom != 1:
        raise ValueError(
            "Four-membered-ring labels are defined for the first atom in --ring. "
            "Rotate --ring so the requested out-of-plane atom is first."
        )
    return "E", atom, 180.0 if opposite else 0.0


def phase_constraint_coefficients(phi_deg: float) -> np.ndarray:
    phi = math.radians(phi_deg)
    qc_coeff, qs_coeff = five_ring_component_coefficients()
...
def phi_values(start: float, stop: float, step: float) -> np.ndarray:
    if step <= 0.0:
        raise ValueError("--phi-step must be positive")
    count = int(round((stop - start) / step))
    values = start + step * np.arange(count + 1, dtype=float)
    if values[-1] < stop - 1.0e-8:
        values = np.append(values, stop)
    if abs(values[-1] - stop) > 1.0e-8:
        values[-1] = stop
    return values


def format_geometry(structure: Structure) -> str:
    lines = []
    for symbol, (x, y, z) in zip(structure.symbols, structure.coords_angstrom):
...
def five_ring_gic_torsion_lines(ring_indices: list[int]) -> tuple[list[str], list[str]]:
    ...
    return [
        f"T0(Inactive)=D({atom[0]},{atom[1]},{atom[2]},{atom[3]})",
        f"T1(Inactive)=D({atom[1]},{atom[2]},{atom[3]},{atom[4]})",
        f"T2(Inactive)=D({atom[2]},{atom[3]},{atom[4]},{atom[0]})",
        f"T3(Inactive)=D({atom[3]},{atom[4]},{atom[0]},{atom[1]})",
        f"T4(Inactive)=D({atom[4]},{atom[0]},{atom[1]},{atom[2]})",
    ], ["T0", "T1", "T2", "T3", "T4"]


def four_ring_gic_definitions(
    ring_indices: list[int],
    target_deg: float,
    current_deg: float,
    *,
    ...
    if constraint_mode == "functional-targets":
        return [
            f"Puck(Inactive)=D({atom[1]},{atom[2]},{atom[3]},{atom[0]})",
            f"PuckStep(NSteps=1,StepSize={step_rad:.10f})=Puck",
        ], step_deg
    if constraint_mode == "freeze-current":
        return [
            f"Puck(Freeze)=D({atom[1]},{atom[2]},{atom[3]},{atom[0]})",
        ], 0.0
    if constraint_mode == "scan-to-zero":
        return [
            f"Puck(NSteps=1,StepSize={step_rad:.10f})=D({atom[1]},{atom[2]},{atom[3]},{atom[0]})",
        ], step_deg
    raise ValueError(
        "Four-membered-ring scans support functional-targets, scan-to-zero, or freeze-current"
    )


def gic_definitions_for_ring(
    ring_indices: list[int],
    phi_deg: float,
    cphase_initial: float,
    *,
    constraint_mode: str,
) -> list[str]:
    torsion_lines, names = five_ring_gic_torsion_lines(ring_indices)
    qc_coeff, qs_coeff = five_ring_component_coefficients()
    phase_coeff = phase_constraint_coefficients(phi_deg)
    phi_label = f"{int(round(phi_deg * 1000.0)):06d}"
    if constraint_mode == "scan-to-zero":
        phase_header = f"Cphi{phi_label}(NSteps=1,StepSize={format_float(-cphase_initial)})"
    elif constraint_mode == "freeze-current":
        phase_header = f"Cphi{phi_label}(Freeze)"
    else:
        raise ValueError(f"Unsupported constraint mode: {constraint_mode}")
    return [
        *torsion_lines,
        f"QC(Inactive)={gic_linear_expression(qc_coeff, names)}",
        f"QS(Inactive)={gic_linear_expression(qs_coeff, names)}",
        f"{phase_header}={gic_linear_expression(phase_coeff, names)}",
    ]


def functional_gic_definitions_for_ring(
    ring_indices: list[int],
    scan_end_deg: float,
    scan_step_deg: float,
) -> list[str]:
    torsion_lines, names = five_ring_gic_torsion_lines(ring_indices)
    qc_coeff, qs_coeff = five_ring_component_coefficients()
    nsteps = int(round(scan_end_deg / scan_step_deg))
    if nsteps <= 0:
        raise ValueError("Functional phase scan requires a positive angular range")
    step_rad = math.radians(scan_step_deg)
    return [
        *torsion_lines,
        f"QC(Inactive)={gic_linear_expression(qc_coeff, names)}",
        f"QS(Inactive)={gic_linear_expression(qs_coeff, names)}",
        "PhiAbs(Inactive)=ATAN2(QS,QC)",
        f"Phi(NSteps={nsteps},StepSize={step_rad:.10f})=PhiAbs",
    ]
...
def functional_target_gic_definitions_for_ring(
    ring_indices: list[int],
    target_phi_deg: float,
    ...
    return [
        *torsion_lines,
        ...
        f"QS(Inactive)={gic_linear_expression(qs_coeff, names)}",
        "PhiAbs(Inactive)=ATAN2(QS,QC)",
        f"Phi(NSteps=1,StepSize={step_rad:.10f})=PhiAbs",
    ], step_deg


def write_gaussian_phi_scan(source: SourceData, args: argparse.Namespace) -> None:
    check_atom_order(source.structures)
    structure_index = args.structure_index
    if structure_index < 0:
        structure_index = len(source.structures) + structure_index
    if structure_index < 0 or structure_index >= len(source.structures):
    ...
    if start_pucker_label:
        if args.start_pucker_mode == "cartesian":
            if len(ring_indices) != 5:
                raise ValueError("--start-pucker-mode cartesian is currently implemented for five-membered rings")
            structure = pucker_structure_from_label(
                structure,
                ring_indices,
                start_pucker_label,
                args.pucker_amplitude,
            )
            structure.props["start_pucker_mode"] = "cartesian"
        else:
            structure = label_structure_for_gic_start(structure, start_pucker_label, len(ring_indices))
    start_label = structure.props.get("start_pucker_label", "")
    start_phi = structure.props.get("start_pucker_phi_deg", "")
    start_mode = structure.props.get("start_pucker_mode", "")
    start_cartesian_phi = structure.props.get("start_pucker_cartesian_phase_deg", "")
    start_q = structure.props.get("start_pucker_q_angstrom", "")
    qc, qs, q, diagnostic_phi = ring_coordinate_components(
        structure.coords_angstrom, ring_indices
    )
    if len(ring_indices) == 5:
        cp_q2, cp_phi2, cp_x, cp_y = cremer_pople_five_membered(
            structure.coords_angstrom, ring_indices
        )
    else:
        cp_q2, cp_phi2, cp_x, cp_y = "", "", "", ""
    phis = phi_values(args.phi_start, args.phi_end, args.phi_step)
    signed_initial_phi = math.atan2(qs, qc)
    initial_abs_phi_deg = diagnostic_phi

    charge = source.charge if args.charge is None else args.charge
    multiplicity = source.multiplicity if args.multiplicity is None else args.multiplicity
    gjf_path = args.gjf_out.expanduser().resolve()
    ...
    phase_convention = (
        "Prelog ring order; phi=0 is the E1 envelope"
        if len(ring_indices) == 5
        else "Four-membered ring; coordinate is D(atom2,atom3,atom4,atom1), atom1 out of the plane"
    )
    if args.constraint_mode == "functional-phase":
        if len(ring_indices) != 5:
            raise ValueError("--constraint-mode functional-phase is implemented only for five-membered rings")
        if abs(args.phi_start) > 1.0e-10:
            raise ValueError(
                "--constraint-mode functional-phase uses the absolute phase of the input "
                "geometry as the scan origin; use --constraint-mode functional-targets "
                "to generate an absolute target grid with arbitrary --phi-start."
            )
        lines.extend(
            [
                f"%chk={args.chk_prefix}.chk",
                f"%mem={args.mem}",
                f"%nprocshared={args.nproc}",
                route,
                "",
                (
                    f"{args.title}: functional phase scan from "
                    f"{args.phi_start:.3f} to {args.phi_end:.3f} deg"
                ),
                "",
                f"{charge} {multiplicity}",
                format_geometry(structure),
                "",
                *functional_gic_definitions_for_ring(
                    ring_indices,
                    args.phi_end - args.phi_start,
                    args.phi_step,
                ),
                "",
            ]
        ...
        manifest_rows.append(
            [
                ...
                q,
                diagnostic_phi,
                math.degrees(signed_initial_phi),
                phase_convention,
                cp_q2,
                cp_phi2,
                cp_x,
        ...
        with manifest_path.open("w", newline="") as handle:
            ...
            writer.writerow(
                [
                    ...
                    "initial_QS_rad",
                    "initial_q_rad",
                    "initial_diagnostic_phi_deg_0_360",
                    "initial_signed_phi_deg",
                    "phase_convention",
                    "initial_CP_q2_angstrom",
                    "initial_CP_phi2_deg",
                    "initial_CP_x_angstrom",
                    "initial_CP_y_angstrom",
                    "start_pucker_label",
                    "start_pucker_phi_deg",
                    "start_pucker_mode",
                    "start_pucker_cartesian_phase_deg",
                    "start_pucker_q_angstrom",
                    "constraint_mode",
                ]
            )
        ...
        print(manifest_path)
        print(
            "After running Gaussian, analyze the log with: "
            "python3 scripts/mw_path_dvr.py --gaussian-log <log> "
            "--log-selection all --boundary periodic --compute-rotconst "
            "--label-cremer-pople"
        )
        return

    if args.constraint_mode == "functional-targets":
        for i, phi in enumerate(phis):
            if len(ring_indices) == 5:
                gic_lines, step_deg = functional_target_gic_definitions_for_ring(
                    ring_indices,
                    float(phi),
                    initial_abs_phi_deg,
                )
            else:
                gic_lines, step_deg = four_ring_gic_definitions(
                    ring_indices,
                    float(phi),
                    initial_abs_phi_deg,
            ...
            lines.extend(
                [
                    f"%chk={args.chk_prefix}_{int(round(phi)):03d}.chk",
                    f"%mem={args.mem}",
                    f"%nprocshared={args.nproc}",
                    route,
                    "",
                    f"{args.title}: absolute phi={phi:.3f} deg",
                    "",
                    f"{charge} {multiplicity}",
                    format_geometry(structure),
                    "",
        ...
        with manifest_path.open("w", newline="") as handle:
            ...
            writer.writerow(
                [
                    "link",
                    "target_absolute_phi_deg",
                    "initial_absolute_phi_deg",
                    "gic_step_deg",
                    "initial_QC_rad",
                    "initial_QS_rad",
                    "initial_q_rad",
        ...
        print(
            "After running Gaussian, analyze the log with: "
            "python3 scripts/mw_path_dvr.py --gaussian-log <log> "
            "--log-selection last-per-link --boundary periodic --compute-rotconst "
            "--label-cremer-pople"
        )
        return
    ...
    for i, phi in enumerate(phis):
        if len(ring_indices) == 5:
            phase_coeff = phase_constraint_coefficients(float(phi))
            cphase_initial = float(phase_coeff @ five_ring_endocyclic_torsions(structure.coords_angstrom, ring_indices))
            step_to_zero = -cphase_initial
            gic_lines = gic_definitions_for_ring(
                ring_indices,
                float(phi),
                cphase_initial,
                constraint_mode=args.constraint_mode,
            ...
            cphase_initial = math.radians(initial_abs_phi_deg)
            step_to_zero = math.radians(step_deg)
        if i:
            lines.append("--Link1--")
        lines.extend(
            [
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--prepare-gaussian",
        action="store_true",
        help="Prepare a Gaussian four- or five-membered-ring scan input instead of running path analysis.",
    )
    parser.add_argument(
        "--log-selection",
    ...
    parser.add_argument("--charge", type=int, help="Molecular charge for generated Gaussian input.")
    parser.add_argument("--multiplicity", type=int, help="Spin multiplicity for generated Gaussian input.")
    parser.add_argument(
        "--structure-index",
        type=int,
        default=-1,
        help="Structure used for Gaussian input generation; negative values count from the end.",
    )
    parser.add_argument(
        "--ring",
        default="1,2,3,4,5",
    ...
    parser.add_argument(
        "--start-pucker-label",
        help=(
            "Nominal starting puckering label. In the default GIC mode, "
            "Cartesian coordinates are left unchanged and Gaussian is driven through the GIC step. "
            "Five-membered rings accept E1, E_1, ^1E, E^1, T1, T_1, ^1T, and T^1; "
            "four-membered rings accept E1 or ^1E for the first atom in --ring. "
            "For --xyz --prepare-gaussian, the default is E1."
        ),
    )
    parser.add_argument(
        "--start-pucker-mode",
        choices=["gic", "cartesian"],
        default="gic",
        help=(
            "How --start-pucker-label is applied. gic keeps the input Cartesians and changes only "
            "the GIC target; cartesian builds a puckered Cartesian guess for non-Gaussian workflows."
        ),
    )
    parser.add_argument(
        "--pucker-amplitude",
        type=float,
        help=(
            "Cartesian puckering amplitude in Angstrom for --start-pucker-mode cartesian. "
            "Default: current Cremer-Pople q2, or 0.35 Angstrom for a planar input."
        ),
    )
    parser.add_argument("--phi-start", type=float, default=0.0)
    parser.add_argument("--phi-end", type=float, default=180.0)
    parser.add_argument("--phi-step", type=float, default=5.0)
    parser.add_argument(
        "--constraint-mode",
        choices=["functional-targets", "functional-phase", "scan-to-zero", "freeze-current"],
        default="functional-targets",
        help="How the GIC phase constraint is imposed in the generated Gaussian input.",
    )
    parser.add_argument("--mem", default="16GB")
    parser.add_argument("--nproc", default="8")
    parser.add_argument("--chk-prefix", default="mw_path_phi")
    parser.add_argument("--title", default="Five-membered-ring phase constrained optimization")
    parser.add_argument("--route", default=DEFAULT_GAUSSIAN_ROUTE)
    parser.add_argument(
        "--gjf-out",
        type=Path,
        default=DEFAULT_GAUSSIAN_INPUTS / "mw_path_phi_scan.gjf",
    )
    parser.add_argument(
        "--manifest-out",
        type=Path,
        default=DEFAULT_GAUSSIAN_INPUTS / "mw_path_phi_scan_manifest.csv",
    )
...
def validate_args(args: argparse.Namespace) -> None:
    extension_mode = canonical_extension_mode(args.potential_extension)
    if args.grid2d_csv and args.prepare_gaussian:
        raise ValueError("--prepare-gaussian cannot be used with --grid2d-csv")
    if args.property_derivatives_csv and (args.grid2d_csv or args.anharmonic_mode is not None or args.prepare_gaussian):
        raise ValueError("--property-derivatives-csv is supported only for 1D path calculations")
    if args.vpt2_property_csv and (args.grid2d_csv or args.anharmonic_mode is not None or args.prepare_gaussian):
    ...
    if args.anharmonic_mode is not None:
        ...
        if args.prepare_gaussian:
            raise ValueError("--prepare-gaussian cannot be used with --anharmonic-mode")
...
def run_1d_mode(args: argparse.Namespace) -> None:
    ...
    if args.prepare_gaussian:
        write_gaussian_phi_scan(source, args)
    else:
