...
- `examples/gaussian_inputs`: reference Gaussian inputs and manifest files
  generated by Merlino/`merlino_fit`.
...
metrics, and runs one-dimensional Gaussian-log and nonperiodic tail-extension
checks. It writes temporary files under
`${TMPDIR:-/tmp}/mw_path_dvr_smoke`.

## Generate Gaussian Input

Gaussian input/path generation is intentionally outside this DVR program. In
Merlino 3.0, use the GUI Advanced puckering workflow or
`merlino_fit pucker-gaussian`; then pass the completed Gaussian log to this DVR
backend for levels, wavefunctions, properties, and optional Cremer-Pople labels.
...
Planned TODOs are higher-dimensional product bases, optional Podolsky
pseudopotential corrections, and native six-/seven-membered-ring puckering
coordinate builders. Gaussian input generation belongs to Merlino/`merlino_fit`;
this DVR package consumes completed Gaussian outputs for the level calculation.
