...
formalized step is the construction of that model. Active coordinates,
...
semiexperimental refinements reported here are the principal validation, but
the construction is more general: it provides an algorithmic way to build
constrained refinement models from topology, symmetry, rank, and external
reference information before any numerical least-squares solution is attempted.
The resulting models are not merely topology-aware: the active representation and
all constraint or parameter-class relations are generated from the perceived
graph, symmetry-consistent, rank-consistent, and recorded. Molecular topology defines
...
check further shows that quantum chemical equilibrium geometries, accelerated
vibrational corrections, and class-constrained model generation can be combined
...
innocent, especially for rings, fused systems, and bridged frameworks. A
molecular graph with cycles is not a tree; different reference paths can make
one distance, angle, or torsion primary and an equivalent one derived. For
such systems, construction of a symmetry-consistent non-redundant refinement
model is itself a major part of the semiexperimental analysis. The target of
the present work is therefore not a new least-squares observable or a
molecule-specific improvement in residuals, but an algorithmic route from
perceived topology to a reproducible statistical refinement model.
Although semiexperimental structure refinement is the principal use case
developed here, the same construction applies more generally to constrained
refinement models in which topology, symmetry, rank, and external information
must be combined before the numerical fit is solved.
...
model itself graph-defined rather than hand specified. Reference geometries from QC
...
prerequisites for comparison with ground-state rotational constants; the
framework adds the missing step of converting those ingredients into an
explicit, rank-audited refinement model without molecule-specific coordinate
design.

The procedure starts from Cartesian coordinates. Primitive GICs are built,
redundancies are removed within homogeneous coordinate families, symmetry is
assigned, and the totally symmetric active space is defined. Isotope coverage,
symmetry orbits, chemical descriptors, and reference data then generate
compatible constraints, predicates, and parameter classes. Two active spaces
are available: symmetry-adapted GICs and symmetry-adapted Cartesians after
removal of translations and rotations. Constraints remain primitive
internal-coordinate relations and are projected onto either active basis by
the Wilson matrix or the corresponding derivative operator.\cite{Wilson1955}
...
combinations describe ring bending, whereas the endocyclic torsional manifold
is reported through ring-puckering descriptors such as $q$ and $\phi$. These
descriptors are constructed from the underlying dihedral combinations but are
more stable and interpretable than individual endocyclic torsions, so
...
decomposition,\cite{GolubReinsch1970}
...
demanding parts of the same model-generation problem. Glycolaldehyde is
...
generated model: topology perception, symmetry analysis, and isotopic coverage
replace molecule-specific coordinate edits. The left panel reports the number
of coordinates generated algorithmically, the symmetry-active subset before
...
for every case. Detailed fit residuals and ranks are collected in the
Supporting Information.}
\label{fig:benchmark-overview}
\end{figure*}

Figure~\ref{fig:benchmark-overview} is the central validation of the method:
...
they are made by topology perception, symmetry assignment, coverage analysis,
and rank checks. Stretches, bends, torsions, out-of-plane terms, linear bends,
and ring-deformation coordinates are generated from the perceived graph and
reduced in homogeneous families. Consequently, a stretching coordinate cannot
replace a torsion or a ring coordinate during rank reduction.
...
\toprule
Case & Manual workflow would decide & Automated model decisions & Generated model & Constraints/classes & Result & Manual edits \\
\midrule
Azulene &
...
constructed from the endocyclic dihedral manifold rather than selected as
...
Ring puckering & \texttt{q, phi = f(D-ring set)} & reports puckering from endocyclic dihedrals \\
\bottomrule
\end{tabular}
\end{table*}
...
\input{generated/sensitivity_leave_one_out}
...
class layer applies a single graph-defined C--C skeletal stretch class.
...
\smallskip
\scriptsize Testosterone large-molecule scale demonstration.
...
The central contribution of this work is a general framework for constructing
constrained refinement models, demonstrated here through semiexperimental
structure refinement. Existing refinement methods generally assume that
...
The benchmarks show that the resulting models remain graph-defined,
...
refinement models.
The framework therefore performs two coupled tasks simultaneously: it
constructs the refinement model algorithmically and refines the initial
equilibrium geometry toward the semiexperimental solution. Model generation
and structural refinement are integrated into a single workflow rather than
being treated as separate steps.
The essential shift is from expert-dependent model formulation to reproducible
algorithmic model generation. The algorithm replaces expert-dependent model
formulation rather than expert chemical judgement.
Although motivated by semiexperimental equilibrium structures, the underlying
graph-driven model-construction strategy is applicable to any constrained
refinement problem requiring simultaneous treatment of topology, symmetry, rank
deficiency, and external structural information.
