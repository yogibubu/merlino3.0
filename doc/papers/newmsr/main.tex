...
The approach extends to semiexperimental refinement the coordinate philosophy
by which geometry optimization moved from minimal Z-matrices to redundant
internal coordinates, with the additional requirement that the active fitting
space be non-redundant and statistically well defined.
...
Geometry optimization solved the analogous coordinate problem by moving from
minimal Z-matrices to redundant internal coordinates rather than by searching
for a unique minimal parameterization. Semiexperimental refinement requires
the same change in philosophy, but the active least-squares space must also be
non-redundant and statistically well defined.
...
The goal is therefore not to obtain a different structure, but to obtain the
same structure from a unique topology-derived coordinate model.
...
The independent Fortran77 numerical-kernel layer implements the same
column-scaled More--Hebden trust-region subproblem. In fixed-form Fortran77
the SVD-equivalent step is obtained from the symmetric eigendecomposition of
the scaled Gram matrix, with the same rank threshold, damping convention,
active-radius bisection and grouped robust weights as the Python driver,
without importing legacy Gaussian code.
...
The standard input model is a versioned self-contained job file containing
Cartesian parent geometry, optional Gaussian-style primitive freeze records,
and isotopologue tables with definitions, ground-state rotational constants,
vibrational and electronic corrections, uncertainties, and correction
conventions. A separate reusable isotopologue file can still be referenced when
convenient. Legacy MSR-style input is accepted for reproducibility, including
frozen variables marked by
...
both cases.
In this sense, \texttt{SEfit} transfers to semiexperimental refinement the
coordinate philosophy that made redundant internal coordinates standard in
geometry optimization, while replacing redundancy by a projected active space
