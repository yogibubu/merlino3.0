...
```bash
merlino multistructure-reference-search \
  --query-xyz path/to/query.xyz \
  --outdir working/multistructure_reference_search/query
```

The command writes a ranked CSV/JSON match list and a Merlino run manifest.
...
```bash
merlino multistructure-build-reference-geometry \
  --query-xyz path/to/query.xyz \
